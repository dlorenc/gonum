...
func (u *urna) relabel(dst, src []*Statement) ([]*Statement, error) {
	...
	for i, s := range src {
		...
		n := dst[i]
		n.Subject = Term{Value: translateURNA(s.Subject.Value, u.canon.issued)}
		n.Predicate = s.Predicate
		n.Object = Term{Value: translateURNA(s.Object.Value, u.canon.issued)}
		n.Label = Term{Value: translateURNA(s.Label.Value, u.canon.issued)}
	}
